...
class ForeignFuturesOIProvider(DataProvider):
    """外資期貨未平倉部位(三大法人-區分各期貨契約-依日期,TAIFEX OpenAPI)。
    ...
    def fetch(self, date_str):
        ...
        try:
            ...
            log.warning(f"外資期貨未平倉抓取失敗: {e}")
            return []

        rows = [d for d in data
                if d.get("Item") == "外資及陸資" and d.get("ContractCode") in CONTRACTS]
        if not rows:
            return []

        actual_date = rows[0]["Date"]
        if actual_date != date_str:
            log.warning(f"外資期貨未平倉 API 僅提供最新資料({actual_date}),"
    ...
    def describe(self, rows):
        if not rows:
            return None
        lines = [f"   外資期貨未平倉({r['trade_date']}) {r['contract_code']}: 淨{r['oi_net']:+,}口"
                  for r in rows]
        return "\n".join(lines)
